...
namespace Bernina.SerialStack
{
    ...
    public class SerialStack : IDisposable
    {
        ...
        public async Task<bool> ChangeTo57600BaudAsync()
        {
            ...
            try
            {
                // Send TrMEJ05 command to tell machine to switch
                var result = await SendCommandAsync("TrMEJ05");
                ...
                // Wait a moment for the machine to settle at new baud rate
                await Task.Delay(200);

                // Re-establish connection with RF? command
        ...
        private void ResetResponseTimeout()
        {
            ...
            _responseTimeoutTimer = new Timer(_ => OnResponseTimeout(), null, _charTimeoutMs, Timeout.Infinite);
        }
        ...
        private async Task<bool> IsResponseCompleteAsync(string command, string response)
        {
